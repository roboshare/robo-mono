...
import { RentalMarketplaceExperience } from "~~/components/robomata/RentalMarketplaceExperience";
import { isRobomataRentalMarketplaceClientEnabled } from "~~/lib/featureFlags";

const RentalsPage = () =>
  isRobomataRentalMarketplaceClientEnabled() ? (
    <RentalMarketplaceExperience />
  ) : (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-10 sm:px-6">
      <div className="rounded-[2rem] border border-base-300 bg-base-100 p-8 text-center shadow-lg shadow-base-300/30">
        ...
        <h1 className="mt-3 text-3xl font-black tracking-tight text-base-content">
          Rental marketplace is not enabled in this environment.
        </h1>
      </div>
    </div>
  );
