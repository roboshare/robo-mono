...
function paymentMatchesEntry(payment: RentalPaymentRecord, entry: RentalRevenueLedgerEntry) {
  ...
  return bookingMatches && intentMatches && facilityMatches && platformVehicleMatches && vehicleMatches;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const revenueStore = getRentalRevenueStore();
    if (isRobomataRentalPaymentsEnabled()) {
      const entriesMissingPaymentReference = body.entries.filter(
        entry => !nonEmptyString(entry.source?.bookingId) && !nonEmptyString(entry.source?.paymentIntentId),
      );
      if (entriesMissingPaymentReference.length > 0) {
        return NextResponse.json(
          {
            entries: entriesMissingPaymentReference.map(entry => ({ id: entry.id })),
            error: "Payment-backed revenue entries must include source.bookingId or source.paymentIntentId.",
          },
          { status: 400 },
        );
      }

      const paymentBackedEntries = body.entries;
      const bookingIds = [
      ...
      return revenueStore.withPaymentBackedPostingLock(
        ...
        async () => {
          ...
          const result = await revenueStore.createPostingBatch({
