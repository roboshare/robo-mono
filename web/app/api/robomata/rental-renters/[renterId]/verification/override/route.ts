...
function requireVerificationAuthorization(request: NextRequest) {
  ...
  if (!configuredSecret) {
    return NextResponse.json({ error: "Renter verification overrides require a configured secret." }, { status: 403 });
  }
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const renter = await getRentalRenterStore().updateVerification(renterId, {
      ...body,
      actor: body.actor ?? { id: body.reviewedBy ?? "unknown-admin", type: "admin", displayName: body.reviewedBy },
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to apply renter verification override." },
